...
def index(request):
    ...
    topics = get_topics_for_user(request.user).order_by('-date_added')[:5]
    context = {'topics': topics}
    return render(request, 'learning_logs/index.html', context)


def get_topics_for_user(user):
...
def topics(request):
    """Show all topics."""
    topics = get_topics_for_user(request.user).order_by('-date_added')
    context = {'topics': topics}
...
def topic(request, topic_id):
    ...
    topics = get_topics_for_user(request.user)
